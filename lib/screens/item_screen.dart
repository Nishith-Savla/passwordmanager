...
class _ItemScreenState extends State<ItemScreen> {
  ...
  void initState() {
    ...
    createdAt = widget.passwordEntry?.createdAt ?? Timestamp.now();

    () async {
      _key = generateKey(await getMasterPassword(), pepper, createdAt);
    }();
  }
  ...
  void _onSave() async {
    if (widget.passwordEntry == null) {
      widget.onSave(PasswordEntry(
        ...
        createdAt: createdAt,
        url: uriController.text,
        key: _key,
        password: passwordController.text,
      ));
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text("New entry for ${nameController.text} created"),
      ));
